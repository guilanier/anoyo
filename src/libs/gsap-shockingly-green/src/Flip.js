...
let _id = 1,
    ...
    _orderByDOMDepth = (comps, invert, isElStates) => {
        comps.forEach(
            (comp) => (comp.d = _getDOMDepth(isElStates ? comp.element : comp.t, invert))
        );
    ...
    _fit = (fromState, toState, scale, applyProps, fitChild, vars) => {
        ...
        if (simple || !parent) {
            ...
            matrix = parentMatrix
                .clone()
                .multiply(
                    toState.ctm ? toState.matrix.clone().multiply(toState.ctm) : toState.matrix
    ...
    _fromTo = (fromState, toState, vars, relative) => {
        ...
        run = () => {
            ...
            _callbacks.forEach(
                (name) =>
                    vars[name] && animation.eventCallback(name, vars[name], vars[name + 'Params'])
    ...
    compare(state) {
        ...
        for (p in l1) {
            ...
            if (s2Alt) {
                ...
                c1 =
                    s1Alt && !s1.isVisible && !s1Alt.isVisible && c2.element === s1Alt.element
                        ? s1Alt
                        : s1;
