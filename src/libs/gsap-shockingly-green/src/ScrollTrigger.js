...
let gsap,
    ...
    _getGSAP = () =>
        gsap || (_windowExists() && (gsap = window.gsap) && gsap.registerPlugin && gsap),
    ...
    _refreshAll = (force, skipRevert) => {
        ...
        _triggers.forEach(
            (t) => t.vars.end === 'max' && t.setPositions(t.start, _maxScroll(t.scroller, t._dir))
...
export class ScrollTrigger {
    ...
    init(vars, animation) {
        ...
        self.getTrailing = (name) => {
            ...
            return (_isString(name) ? a.filter((t) => t.vars.preventOverlaps === name) : a).filter(
                (t) => (self.direction > 0 ? t.end <= start : t.start >= end)
            );
        };
            ...
            if (clipped !== prevProgress && self.enabled) {
                ...
                if (stateChanged && !_refreshing) {
                    ...
                    if (toggled || !_limitCallbacks) {
                        ...
                        once &&
                            (clipped === 1 ? self.kill(false, 1) : (callbacks[toggleState] = 0)); // a callback shouldn't be called again if once is true.
                        if (!toggled) {
