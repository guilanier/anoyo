...
#endif`;class Ze extends Nn{constructor(n,e){n=Object.assign({vertexShader:Qe,fragmentShader:$e,type:"LensMaterial",extensions:{derivatives:!0},transparent:!0,depthTest:!1,depthTest:!1},n),super(n),this.defines={HAS_REVERSE:!1,HAS_MASKING:!1,HAS_BLENDING:!1,CENTER_ALIGN:!0,LOW_RES:!1,USE_DEBUG:!1,...n.defines};const A={u_time:{value:0},u_alpha:{value:0},u_progressBlur0:{value:0},u_progressBlur1:{value:0},u_pointer:{value:new l},u_pointerBlur:{value:.25},u_color:{value:new b("#000000")},u_colorBlending:{value:new b("#ffffff")}};this.uniforms={...A,...this.uniforms},Ee(this)}}const Qe=`
    varying vec2 vUv;
...
`,Je={name:"TextLens",props:{id:{type:Number,default:Date.now()},text:{type:String,default:""},width:{type:Number,default:1/0},align:{type:String,default:"center"},blending:{type:Number,default:te},color:{type:String,default:"#ffffff"},lowQuality:{type:Boolean,default:!1},focused:{type:Boolean,default:!0},placeholder:{type:Boolean,default:!1}},emits:["text:focus","text:unfocus"],setup(E,{emit:n}){const e=K({posInner:new o(0,0,0),posOffset:new o(0,0,0),aBlur:0,aAlpha:1,sBase:Ce?22.2:13.4,sZoom:1,pSizePointerBlur:.25}),{renderer:A}=h("renderer"),i=vn(({width:a,height:m})=>{const F=A.getPixelRatio();D.set(a*F,m*F)},{immediate:!0}),f=K({});be(({data:a})=>{f.fontMap=a.fontMap,f.fontData=a.fontData,gn()});const M=kn(()=>e.sBase*e.sZoom*(16/1920*i.width)),{object:d}=ie(null,{props:{s:M}}),T=new ae,t=new Se;let C=null,P=null;const u=new o,S=new o,L=new o(.5,0,0),O=new l,dn=new o().copy(e.posOffset),D=new l,_n=new l,pn=new b(E.color),{set:Ln}=q(u,{lambda:1.4}),{set:Gn}=q(e,{lambda:6},["pSizePointerBlur"]),{set:hn}=q(_n,{lambda:8},["x","y"]);let v;const Vn=async()=>{const a={tMap:{value:f.fontMap},uBounds:{value:S},uResolution:{value:D},u_pointer:{value:_n},u_color:{value:pn}};v=new Ze({uniforms:a,defines:{HAS_MASKING:!0,CENTER_ALIGN:E.align==="center",LOW_RES:E.lowQuality},blending:E.blending}),P=new un(t,v),P.layers.set(1),T.add(P)},k=a=>{if(!C)return;C.update({text:a});const{position:m,uv:F,id:Xn,index:yn}=C.buffers;t.setAttribute("position",new W(m,3)),t.setAttribute("uv",new W(F,2)),t.setAttribute("id",new W(Xn,1)),t.setIndex(new oe(yn,1)),t.computeBoundingBox(),t.boundingBox.getSize(S),Ln({x:Math.max(L.x-(S.x-L.x),0)},!0),L.copy(S),O.set(C.width,C.height),P.position.set(0,S.y*(S.y/C.height),0),Ln(S)},{isDown:Hn}=re(({x:a,y:m})=>{const F=A.getPixelRatio();hn({x:a*F,y:D.y-m*F})});U(Hn,a=>{Gn({pSizePointerBlur:a?.5:.25})}),U(e.posInner,a=>T.position.copy(a).add(dn),{immediate:!0});const Rn=()=>{const{aAlpha:a,aBlur:m}=e;v&&(v.u_progressBlur0=m,v.u_progressBlur1=E.placeholder?1:S.x==0?0:Math.max(0,u.x/S.x),v.u_pointerBlur=e.pSizePointerBlur,v.u_alpha=a)},gn=()=>{C=new fe({font:f.fontData,text:E.text,width:E.width,align:E.align,letterSpacing:E.letterSpacing,lineHeight:E.lineHeight,maxTimes:120}),d.add(T),Vn(),k(E.text),Rn()};let c;const Tn=()=>{c==null||c.kill(),E.placeholder?c=rn.timeline().fromTo(e,{aAlpha:0},{aAlpha:1,duration:3},.1).fromTo(e,{sZoom:1.6},{sZoom:1,duration:2.4,ease:"sine.out"},0).fromTo(e,{aBlur:0},{aBlur:1,duration:4,ease:"power2.out"},.2):an()},tn=()=>{c==null||c.kill(),c=rn.timeline({onComplete:()=>n("text:unfocus",{id:E.id})}).to(e,{aAlpha:0,duration:1.4},.1).to(e,{sZoom:.7,duration:1.6,ease:"power3.out"},0).to(e,{aBlur:0,duration:2.5,ease:"power2.out"},0)},an=()=>{e.aBlur=1,e.aAlpha=1,e.sZoom=1};Ie(Rn),U(()=>E.text,a=>{c==null||c.kill(),E.focused&&(a==""&&(L.set(.5,0,0),S.set(0,0,0)),an(),k(a))}),U(()=>E.focused,a=>a?Tn():tn(),{immediate:!0}),U(()=>E.placeholder,a=>{a||(L.set(.5,0,0),S.set(0,0,0),k(E.text))});const Sn=fn([],{title:"Text",expanded:!0});return Sn.addButton({title:"focus"}).on("click",()=>Tn()),Sn.addButton({title:"unfocus"}).on("click",()=>tn()),{object:d}}};function nA(E,n,e,A,i,f){return ln(E.$slots,"default")}const eA=zn(Je,[["render",nA]]),AA=16,EA={__name:"index",setup(E){const{renderer:n,scene:e,registerRenderFn:A,orthoCamera:i}=h("renderer"),f=Xe({concurrency:10,loaders:[Un,w]});w.setGlobals({renderer:n});const M=Z(null),d=Z([{id:Date.now(),text:"TYPE.",focused:!1}]),T=Pn(""),t=()=>{d.value[d.value.length-1]&&(d.value[d.value.length-1].focused=!1),d.value.push({id:Date.now(),focused:!0,placeholder:!1}),T.value=""},C=()=>{M.value.focus()},P=S=>{S.key==="Enter"&&t()},u=({id:S})=>{const L=d.value.findIndex(O=>O.id===S);L!==-1&&d.value.splice(L,1)};return U(T,S=>{S.length>=AA&&t(),S.length>0&&d.value[0].placeholder==!0&&(d.value[0].placeholder=!1)}),fn([],{title:"Title Lens",expanded:!0}),En(()=>{f.start(),f.lock(),d.value[0].focused=!0,d.value[0].placeholder=!0,setTimeout(()=>{M.value.focus()},1500)}),A(()=>{n.clear(),n.render(e,i)}),(S,L)=>(V(),Q("div",{class:"textLens",onClick:C},[Wn(jn("input",{ref_key:"refInput",ref:M,class:"textLens__input",onKeyup:P,"onUpdate:modelValue":L[0]||(L[0]=O=>T.value=O),placeholder:""},null,544),[[qn,T.value]]),$(nn(Ye),null,{default:J(()=>[(V(!0),Q(Kn,null,Zn(d.value,(O,dn)=>{var D;return V(),cn(eA,Qn({key:O.id,ref_for:!0},O,{text:O.placeholder?"TYPE.":O.focused?T.value.toUpperCase():(D=T.value.value)==null?void 0:D.toUpperCase(),"onText:unfocus":u}),null,16,["text"])}),128))]),_:1})]))}},iA={class:"component"},tA={__name:"ComponentGl",setup(E){const n=$n(),e=Z(),A=Pn(),f=[{id:"sphere-space-neon",component:he},{id:"text-lens",component:EA}].find(M=>new RegExp(`\\b${M.id}\\b`,"i").test(n.path));return A.value=f.component,En(()=>document.body.style.overflow="hidden"),(M,d)=>(V(),Q("div",iA,[$(nn(ne),{hiddenOnStart:!0},{default:J(()=>[$(nn(Me),{ref:"refRenderer",antialias:!1,autoResize:!0,autoRender:!0},{default:J(()=>[(V(),cn(Jn(A.value),{ref_key:"refComponent",ref:e},null,512))]),_:1},512)]),_:1})]))}};export{tA as default};
